...
# load_dotenv() called at module top
sys.path.append(str(Path(__file__).parent))

from src.database.moderation_db import TalentDecisionDatabase, ModerationDatabase
from src.database.auth_db import AuthDatabase
from src.memory.memory import TalentDecisionMemoryManager, ModerationMemoryManager
from src.agents.workflow import (
    create_talent_decision_workflow, create_moderation_workflow,
    process_talent_decision, process_content,
    resume_from_reflection, resume_from_hitl,
)
from src.core.models import (
    TalentDecisionState, ContentState,
    TalentDecisionStatus, ContentStatus,
    UserProfile, ContentMetadata,
    ReflectionTriggerReason, HITLTriggerReason,
    REFLECTION_CONFIG, HITL_CONFIG,
)
from src.api.talent_routes import router as talent_router, init_talent_routes
from src.api.hiring_routes import router as hiring_router, init_hiring_routes
...
# Global instances
db: TalentDecisionDatabase = None
...
# In-memory store for pending manager reflections (in production, use Redis or database)
reflection_pending_reviews: Dict[str, TalentDecisionState] = {}
hitl_pending_reviews = reflection_pending_reviews  # legacy alias
...
async def lifespan(app: FastAPI):
    ...
    # Initialize workflow
    logger.info("Creating talent decision workflow...")
    quick_check_enabled = os.getenv("ENABLE_QUICK_DECISION_CHECK",
                                     os.getenv("ENABLE_FAST_MODE", "true")).lower() == "true"
    logger.info(f"   Quick Decision Check: {'Enabled' if quick_check_enabled else 'Disabled'}")
    if quick_check_enabled:
        max_length = os.getenv("QUICK_CHECK_MAX_LENGTH", os.getenv("FAST_MODE_MAX_LENGTH", "300"))
        decision_types = os.getenv("QUICK_CHECK_DECISION_TYPES",
                                   os.getenv("FAST_MODE_CONTENT_TYPES", "quick_manager_note"))
        logger.info(f"   Quick Check Max Length: {max_length} chars")
        logger.info(f"   Quick Check Decision Types: {decision_types}")
    workflow = create_talent_decision_workflow(db, enable_quick_check=quick_check_enabled)
    init_talent_routes(db, workflow)
    init_hiring_routes(db)
    ...
    # Shutdown
    logger.info("\nShutting down Clarity API")


app = FastAPI(
    title="Clarity API",
    description="AI manager decision-support for workforce talent decisions, bias detection, and pattern learning",
    version="2.0.0",
    lifespan=lifespan
)
...
async def root():
    ...
    return {
        "message": "Clarity API",
        "description": "Manager decision-support for hire/promote/upskill, bias review, and pattern learning",
        "version": "2.0.0",
        "status": "operational"
    }
