"""
Memory management using ChromaDB for Clarity.

Stores and retrieves:
- Historical talent decisions
- Manager bias patterns
...
class TalentDecisionMemoryManager:
    """Manages persistent memory for Clarity using ChromaDB."""
    ...
    def _init_collections(self):
        ...
        self.patterns_collection = self.client.get_or_create_collection(
            name="manager_bias_patterns",
            metadata={"description": "Recurring manager bias patterns"},
        )
        self.user_history_collection = self.client.get_or_create_collection(
            name="manager_decision_history",
            metadata={"description": "Manager decision history"},
        )
        self.resume_collection = self.client.get_or_create_collection(
            name="resume_screening_decisions",
            metadata={"description": "Resume screening decision history"},
        )
    ...
    def store_talent_decision(
        ...
        decision_context: Optional[str] = None,
        confidence: Optional[float] = None,
        was_reconsidered: bool = False,
        reconsideration_outcome: Optional[str] = None,
        gap_analysis: Optional[List[Dict]] = None,
        manager_reasoning: str = "",
        manager_override_reason: str = "",
        bias_categories: Optional[List[str]] = None,
        manager_pattern_score: float = 0.0,
        final_decision: str = "",
        **kwargs,
    ):
        """Store a talent decision with workforce-specific metadata."""
        self.store_moderation_decision(
            content_id=decision_id,
            content_text=decision_text,
            user_id=manager_id,
            action=action,
            violations=violations or bias_categories or [],
            toxicity_score=bias_risk_score,
            agent_decisions=agent_decisions,
            primary_agent=primary_agent,
            decision_context=decision_context,
            confidence=confidence,
            was_appealed=was_reconsidered,
            appeal_outcome=reconsideration_outcome,
        )
        try:
            extra_meta = {
                "decision_id": decision_id,
                "manager_id": manager_id,
                "gap_analysis": json.dumps(gap_analysis or []),
                "manager_reasoning": manager_reasoning,
                "manager_override_reason": manager_override_reason,
                "bias_categories": json.dumps(bias_categories or []),
                "manager_pattern_score": manager_pattern_score,
                "final_decision": final_decision,
                "timestamp": datetime.now().isoformat(),
            }
            self.decisions_collection.update(ids=[decision_id], metadatas=[extra_meta])
        except Exception:
            pass

    def retrieve_similar_decisions_for_agent(self, *args, **kwargs):
        return self.retrieve_similar_content_for_agent(*args, **kwargs)

    def get_manager_decision_history(self, manager_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        history = self.get_user_history(manager_id, limit)
        decisions = self.retrieve_with_filters(
            content_text=f"manager {manager_id} workforce decision",
            user_id=manager_id,
            n_results=limit,
        )
        return decisions or history

    def update_reconsideration_outcome(self, decision_id: str, outcome: str):
        return self.update_decision_appeal_outcome(decision_id, outcome)
...
ModerationMemoryManager = TalentDecisionMemoryManager
